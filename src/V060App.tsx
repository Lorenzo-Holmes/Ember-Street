...
const CONDITION_LABEL: Record<SurvivorCondition, string> = { healthy: '健康', fatigued: '疲劳', minor: '轻伤', serious: '重伤', critical: '危重', missing: '失踪', dead: '死亡' };
...
function InventoryBar({ state }: { state: GameState }) {
  return <section className="v6-inventory" aria-label="物资箱"><div className="v6-inventory__title"><span>📦 物资箱</span><small>救回的人也需要吃饭</small></div><div className="v6-resource-grid">
    <div><span>口粮</span><b>{state.inventory.ration}</b></div><div><span>药品</span><b>{state.inventory.medicine}</b></div><div><span>电力</span><b>{state.inventory.power}</b></div><div><span>材料</span><b>{state.inventory.materials}</b></div><div><span>零件</span><b>{state.inventory.parts}</b></div><div><span>希望</span><b>{state.hope}</b></div><div><span>防线</span><b>{Math.round(state.defense)}</b></div><div><span>居民</span><b>{population(state)}</b></div>
  </div>{!!state.storyItems.length && <div className="v6-story-items"><strong>特殊物品</strong>{state.storyItems.map((item) => <span key={item}>{item}</span>)}</div>}</section>;
}

function StreetVisual({ state }: { state: GameState }) {
  return <section className={`v6-street v6-street--stage-${state.mainLightStage}`}><div className="v6-street__sky"/><div className="v6-building v6-building--left"/><div className="v6-building v6-building--right"/><div className="v6-main-light"><i/><strong>主灯 · 阶段 {state.mainLightStage}</strong></div><div className="v6-street__meta"><span>居民 {population(state)}</span><span>核心人物 {corePresent(state)}</span><span>DAY {state.day}/30</span><span>{state.forecast.title}</span></div></section>;
}

function BuildingsPanel({ state, setState }: { state: GameState; setState: (state: GameState) => void }) {
  return <section className="v6-section"><div className="v6-section__head"><div><span>街区建设</span><h2>把物资变成能救命的设施</h2></div><small>每座建筑最高 Lv3</small></div><div className="v6-buildings">{BUILDING_IDS.map((id) => {
...
function MissingPanel({ state, setState }: { state: GameState; setState: (state: GameState) => void }) {
  ...
  return <section className="v6-section"><div className="v6-section__head"><div><span>失踪者</span><h2>今天要不要去找他们？</h2></div><small>连续两次搜救失败可能确认死亡</small></div><div className="v6-survivors">{missing.map((s) => {
    const attempted = state.storyFlags.includes(`missing_search:${s.id}:${state.day}`);
    return <article className="v6-survivor" key={s.id}><div className="v6-survivor__top"><div><h3>{s.name}</h3><span>昨晚以前，他/她还在这条街上。</span></div><div><b>?</b><small>状态</small></div></div><p>{attempted ? '今天已经寻找过一次。' : '派两人寻找会占用他们今天的行动；广播搜救消耗 5 点电力。'}</p><div className="v6-job-grid"><button disabled={attempted} onClick={() => commit(searchForMissing(state, s.id, 'team'), setState)}>派两人找</button><button disabled={attempted || state.buildings.radio <= 0 || state.inventory.power < 5} onClick={() => commit(searchForMissing(state, s.id, 'radio'), setState)}>广播搜救</button></div></article>;
  })}</div></section>;
}
...
function DawnScreen({ state, setState }: { state: GameState; setState: (state: GameState) => void }) {
  return <main className="v6-shell v6-shell--dawn"><header className="v6-page-head"><span>DAWN · DAY {state.day}</span><h1>{state.day === 29 ? '最后的夜结束了。' : '天亮了。'}</h1><p>{state.nightState.hordeActive ? '尸潮退去以后，街道重新有了颜色。现在才看得清昨夜留下的损失。' : '发电机的声音重新盖过远处的脚步。今天仍然有事要做。'}</p></header><InventoryBar state={state}/><section className="v6-stats"><div><span>夜间事件</span><b>{state.nightState.resolutions.length}</b></div><div><span>死亡</span><b>{state.campaignStats.deaths}</b></div><div><span>失踪</span><b>{state.campaignStats.missing}</b></div><div><span>救回</span><b>{state.campaignStats.rescued}</b></div></section><MemorialPanel state={state}/><button className="v6-cta" onClick={() => commit(advanceCampaignDay(state), setState)}>{state.day === 29 ? '进入 DAY 30 · 结算' : `开始 DAY ${state.day + 1}`}</button></main>;
}
...
function EndingScreen({ state, meta, onRestart }: { state: GameState; meta: MetaProgress; onRestart: () => void }) {
  ...
  return <main className={`v6-ending v6-ending--${ending.tier}`}><div className="v6-ending__day">DAY 30</div><p>天亮了。</p><section className="v6-ending__ledger"><span>过去 29 天</span><div><b>{state.campaignStats.rescued}</b><small>救回的人</small></div><div><b>{population(state)}</b><small>仍在街区</small></div><div><b>{state.campaignStats.deaths}</b><small>确认死亡</small></div><div><b>{state.campaignStats.expeditions}</b><small>探索次数</small></div><div><b>{state.campaignStats.locationsDiscovered}</b><small>发现地点</small></div><div><b>{state.finalHordeResult ? RESULT_LABEL[state.finalHordeResult] : '未知'}</b><small>DAY 29</small></div></section><MemorialPanel state={state}/><section className="v6-ending__story"><span>{ending.tier === 'secret' ? '隐藏结局' : '结局'}</span><h1>《{ending.title}》</h1><p>{ending.summary}</p></section><section className="v6-ending-gallery"><div><span>结局记录</span><strong>{meta.endingsUnlocked.length}/13</strong></div><div className="v6-ending-grid">{(Object.keys(ENDINGS) as EndingId[]).map((id) => <div className={meta.endingsUnlocked.includes(id) ? 'unlocked' : ''} key={id}><b>{meta.endingsUnlocked.includes(id) ? ENDINGS[id].title : '？？？？'}</b><small>{meta.endingsUnlocked.includes(id) ? ENDINGS[id].tier : endingHint(id)}</small></div>)}</div></section><button className="v6-cta" onClick={onRestart}>开始新的 30 天</button></main>;
}
