...
export const PRINCIPLE_DECISIONS: StreetPrincipleDecision[] = [
  ...
  },
  {
    ...
    choices: [
      ...
      { id: 'preserve-strength', title: '保存力量', detail: '减少非必要消耗，让人先活过今天。', effect: '休息恢复增强，但探索额外收益 -1。' },
    ],
  },
  {
    day: 21,
...
export function principleTitle(id: StreetPrincipleId): string {
  return PRINCIPLE_DECISIONS.flatMap((decision) => decision.choices).find((choice) => choice.id === id)?.title ?? id;
}
