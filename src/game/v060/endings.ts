...
export interface MetaProgress {
  ...
  totalRuns: number;
  bestFinalHordeResult: FinalHordeResult | null;
}

export const ENDINGS: Record<EndingId, EndingDefinition> = {
  E01: { id: 'E01', title: '黎明车队', tier: 'good', hint: '有人一直在监听无线电。', summary: '清晨的雾里出现了车灯。军方终于确认了余烬长街的位置，伤员和居民被分批送上车。老周最后一次关掉主灯——这一次，不再需要它替任何人带路。' },
  ...
  E03: { id: 'E03', title: '第二个灯塔', tier: 'good', hint: '一盏灯能让很远的人看见。', summary: '广播之后，先是两个人，后来是五个人。更远的街区也开始回信。余烬长街没有等到一个救世主，它自己变成了别人寻找的方向。' },
  ...
  E05: { id: 'E05', title: '我们留下', tier: 'normal', hint: '明天依然会有工作。', summary: '没有奇迹，也没有彻底的胜利。DAY 31 的清晨，搜索站照常开门，工坊重新响起敲击声。有人问今天该去哪里找东西——于是大家继续活下去。' },
  E06: { id: 'E06', title: '向南', tier: 'normal', hint: '知道一条路，有时比守住一堵墙更重要。', summary: '最终尸潮留下的损伤太重。天亮以后，大家沿着此前确认的撤离路线向南走。最后一个人回头时，主灯仍在晨雾里亮着。' },
  E07: { id: 'E07', title: '最后一次广播', tier: 'normal', hint: '声音也可以留下证据。', summary: '广播亭撑到了最后。撤离前，小满把最后一段话送了出去：余烬长街即将关闭，这里曾经有人，也有人活了下来。然后她切断电源。' },
  ...
  E10: { id: 'E10', title: '空街', tier: 'bad', hint: '一条街可以完好无损，却已经没有人。', summary: '没有新的敲门声。搜索站空着，工坊空着，诊疗站也空着。主灯仍然亮着，只是灯下已经没有谁需要它了。' },
...
const highTrustCount = (state: GameState) => present(state).filter((s) => (s.trust ?? 0) >= 2).length;
const coreAliveCount = (state: GameState) => present(state).filter((s) => ['lin-xia', 'zhou', 'ahe', 'cheng', 'aliang', 'xiaoman'].includes(s.id)).length;

export function resolveEnding(state: GameState): EndingResult {
  const final = state.finalHordeResult ?? 'damaged';
  const residents = present(state).length;
  const rescued = state.campaignStats.rescued;
  const trust = highTrustCount(state);
  const coreAlive = coreAliveCount(state);
  const built = buildingSum(state);
  const radio = state.buildings.radio;
  const held = final === 'perfect' || final === 'held';
  let id: EndingId = 'E05';

  if (final === 'perfect' && coreAlive >= 6 && rescued >= 8 && built >= 14 && radio >= 3 && state.hope >= 75 && trust >= 5 && flag(state, 'kept_main_light_on')) id = 'E13';
  ...
  else if (residents <= 2 && flag(state, 'kept_main_light_on')) id = 'E12';
...
export function defaultMetaProgress(): MetaProgress {
  return { endingsUnlocked: [], totalRuns: 0, bestFinalHordeResult: null };
}
...
export function loadMetaProgress(): MetaProgress {
  ...
  try {
    const parsed = JSON.parse(localStorage.getItem(META_KEY) ?? 'null') as MetaProgress | null;
    return parsed ? { ...defaultMetaProgress(), ...parsed } : defaultMetaProgress();
