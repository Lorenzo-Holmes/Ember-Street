...
export function buildingEventWeightModifier(state: GameState, event: V060NightEvent): number {
  ...
  if (state.buildings.watchPost >= 3 && ['emergency-north-breach'].includes(event.id)) modifier -= 1;

  if (state.buildings.radio >= 2 && event.category === 'world') modifier += 1;
  if (state.buildings.radio >= 3 && ['water-on-radio'].includes(event.id)) modifier += 1;
