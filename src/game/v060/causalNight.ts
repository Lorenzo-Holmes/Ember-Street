...
const POWER_FAILURE_EVENTS = ['generator-drop', 'clinic-blackout', 'water-on-radio'] as const;
...
export function nightCausalSignals(state: GameState): string[] {
  ...
  const generatorBackup = state.storyFlags.includes('generator_backup');
  ...
  if (state.inventory.power < 35) {
    signals.push(generatorBackup
      ? '电力吃紧，但探索带回的备用发电组件正在降低断电事故风险'
      : '电力吃紧：断电与设备故障更容易发生');
  } else if (generatorBackup) {
    signals.push('备用发电组件已就位：发电机与线路故障风险有所下降');
  }
...
export function nightEventWeight(state: GameState, event: V060NightEvent): number {
  ...
  const communityRepair = community.supportMode === 'repair' && community.repairDefense > 1;
  const generatorBackup = state.storyFlags.includes('generator_backup');
  const powerFailureEvent = POWER_FAILURE_EVENTS.includes(event.id as typeof POWER_FAILURE_EVENTS[number]);
  ...
  if (state.inventory.power < 35 && powerFailureEvent) weight += generatorBackup ? 1 : 3;
  if (generatorBackup && powerFailureEvent) weight -= 1;
