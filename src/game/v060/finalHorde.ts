...
export const FINAL_HORDE_EVENTS: V060NightEvent[] = [
  {
    id: 'final-horde-north-gate', category: 'horde', visualKey: 'night_external_threat', audioKey: 'night_horde_impact', minDay: 29, maxDay: 29,
    title: '第一阵 · 北门',
    body: '第一波尸群没有试探，直接撞向北门。现在要决定谁去顶、要不要把仓房里的东西全压上去，还是把人撤进第二道门。',
    choices: [
  checked('final-gate-hold', '带人顶住北门', '把人放在最前面。街口岗和居民轮值都能帮上忙，但真守不住，最先受伤的也是站在门前的人。', 'watch',
    ...
    resource('final-gate-reinforce', '把材料全压到门上', '把木料、铁件和备件钉到门后，不再让人站到最前面硬撑。北仓库的东西还在的话，这里能少用一些。', { materials: 4, parts: 1 }, { defense: 9, addFlags: ['final_gate_reinforced'] }),
  ...
  {
    id: 'final-horde-power-grid', category: 'horde', visualKey: 'night_power_failure', audioKey: 'night_power_failure', minDay: 29, maxDay: 29,
    title: '第二阵 · 主灯熄灭',
    body: '撞击让主灯线路跳闸。工坊里还有备件，老线路也还能抢修；另一种办法，是干脆熄掉外围，把电留给最不能黑的地方。',
    choices: [
    checked('final-grid-repair', '在黑暗里抢修', '让维修的人摸着发热的线路把主灯重新接回来。线路接上，就能省下备件；没接住，维修的人也会被拖在黑里。', 'repair',
      ...
      resource('final-grid-parts', '整组换掉烧毁组件', '直接拿备件换掉最危险的一段。以前带回发电机或车辆部件的话，这里能少用一些。', { parts: 3 }, { power: 18, defense: 4, addFlags: ['final_grid_replaced'] }),
  ...
  {
    id: 'final-horde-clinic', category: 'horde', visualKey: 'night_return_injured', audioKey: 'night_injured_return', minDay: 29, maxDay: 29,
    title: '第三阵 · 伤员挤进诊疗室',
    body: '围栏后的伤员开始挤满诊疗角。今晚没有“都照顾好”这种选项：要靠程医生分诊，要打开药箱，或者先让还能走的人回到防线上。',
    choices: [
    checked('final-clinic-triage', '让医疗人员连续分诊', '把最危险的人先挑出来。分得及时，能先稳住一名重伤员；来不及，时间就会耗在拥挤里。', 'medical',
        { hope: 4, addFlags: ['final_clinic_triaged'] },
        { hope: 1, addFlags: ['final_clinic_stretched'] },
        { hope: -3, addFlags: ['final_clinic_overwhelmed'] }),
    resource('final-clinic-supplies', '打开应急药箱', '直接用药品先稳住最危险的两个人。过去找到的医疗储备和分诊原则能省下一些药。', { medicine: 2 }, { hope: 4, addFlags: ['final_clinic_supplied'] }),
  ...
  {
    id: 'final-horde-community', category: 'horde', visualKey: 'night_conflict', audioKey: 'night_conflict_murmur', minDay: 29, maxDay: 29,
    title: '第四阵 · 街里开始慌乱',
  ...
  },
  {
    ...
    choices: [
      checked('final-route-scout', '按旧记录带人改线', '让熟悉路线的人判断尸群真正会从哪里进来。墙上的路记得越全，越不容易看错。', 'search',
  ...
  {
    id: 'final-horde-last-line', category: 'horde', visualKey: 'night_shelter_damage', audioKey: 'night_structure_creak', minDay: 29, maxDay: 29,
    title: '第六阵 · 最后一条线',
    body: '天边已经发白。要么让所有还能站的人守住这里，要么搬空仓房，要么承认外层守不住，先把人带进去。',
    choices: [
      checked('final-last-hold', '所有还能站的人一起守住', '人都压上去，最有可能守住，也最可能有人倒下。以前留下的人情和准备，今晚都要在这里见分晓。', 'watch',
    ...
    resource('final-last-stockpile', '把最后的建材和零件全部用掉', '不再让人站到最危险的地方，把仓房里能钉、能焊、能顶住门的东西全部搬出来。', { materials: 6, parts: 3 }, { defense: 13, hope: 2, addFlags: ['final_last_line_fortified'] }),
    consequence('final-last-retreat', '退进内街，先把人保住', '主动承认外层守不住。少守一段街，也少让一个人留在最外面。', { defense: -5, hope: 1, addFlags: ['final_last_line_retreated'] }),
    ],
  },
];
...
export function finalHordeCheckModifiers(state: GameState, choiceId: string): CheckModifier[] {
  ...
  if (choiceId === 'final-community-calm') {
    ...
    if (social.pressure >= 5) modifiers.push({ label: '街区秩序已失控', value: -1 });
