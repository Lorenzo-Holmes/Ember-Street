 ...
 *
 * Important:
 * - A-numbers are production identifiers only. Player-facing UI must use title/gameplay copy.
 * - Paths describe the final offline package layout; binary art import is a separate step.
 * - Assets marked needs-correction must never be shipped until the corrected master is approved.
 * - A19 remains intentionally unresolved here rather than guessing its identity.
 */
export const CANONICAL_VISUAL_ASSETS: readonly VisualAsset[] = [
  ...
  { canonicalId: 'A20', kind: 'event', title: '门后有人', gameplayId: 'apartment-door-402', continuityId: 'apartment-402', path: canonicalPath('A20', 'behind-apartment-door'), status: 'locked' },
...
export const UNRESOLVED_CANONICAL_IDS = ['A19'] as const;
