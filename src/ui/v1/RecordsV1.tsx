...
import { characterVisual, locationVisual } from '../visualAssets';
...
function MiniArt({ src, label }: { src?: string; label: string }) {
  return <div className="v1r-mini-art">{src ? <img src={src} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : null}<span>{label.slice(0, 1)}</span></div>;
}
...
export default function RecordsV1({ state, onBack }: RecordsV1Props) {
  ...
  return (
    <main className="v1r-page">
      ...
      {tab === 'places' && <section className="v1r-grid"><header><span>已发现地点</span><small>{discovered.length}/{EXPEDITION_LOCATIONS.length}</small></header>{discovered.map((location)=>{const art=locationVisual(location.id);return <article key={location.id} className="v1r-place"><MiniArt src={art?.path} label={location.name}/><div><h2>{location.name}</h2><p>{location.description}</p><small>危险 {location.danger}/5 · 主要资源：{location.primary}</small></div></article>})}</section>}

      {tab === 'profiles' && <section className="v1r-profiles"><header><span>角色档案</span><small>人物加入或相关事件发生后逐步展开</small></header>{profiles.map((survivor)=>{const art=characterVisual(survivor.id);const stories=seenCharacterStories.filter((event)=>event.survivorId===survivor.id);return <article key={survivor.id}><div className="v1r-profile-head"><MiniArt src={art?.path} label={survivor.name}/><div><h2>{survivor.name}</h2><span>{survivor.trait ?? survivor.perk}</span><small>当前状态：{survivor.condition ?? 'healthy'} · 信任 {survivor.trust ?? 0}</small></div></div>{stories.length ? <div className="v1r-profile-stories">{stories.map((story)=><section key={story.id}><strong>{story.title}</strong><p>{story.body}</p></section>)}</div> : <p className="v1r-empty">关于这个人的更多故事还没有在本局发生。这里不提前泄露未解锁背景。</p>}</article>})}</section>}
