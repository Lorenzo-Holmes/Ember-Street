...
export default function RecordsV1({ state, onBack }: RecordsV1Props) {
  ...
  return (
    <main className="v1r-page">
      ...
      {tab === 'profiles' && <section className="v1r-profiles"><header><span>角色档案</span><small>认识得越久，留下来的事越多</small></header>{profiles.map((survivor)=>{const art=characterVisual(survivor.id);const stories=seenCharacterStories.filter((event)=>event.survivorId===survivor.id);return <article key={survivor.id}><div className="v1r-profile-head"><MiniArt asset={art} label={survivor.name}/><div><h2>{survivor.name}</h2><span>{survivor.trait ?? survivor.perk}</span><small>当前状态：{SURVIVOR_CONDITION_LABEL[survivor.condition ?? 'healthy']} · 信任 {survivor.trust ?? 0}</small></div></div>{stories.length ? <div className="v1r-profile-stories">{stories.map((story)=><section key={story.id}><strong>{story.title}</strong><p>{story.body}</p></section>)}</div> : <p className="v1r-empty">关于这个人的更多事，还没有在这条街上被说起。</p>}</article>})}</section>}

      {tab === 'memorial' && <section className="v1r-memorial"><header><span>纪念墙</span><small>{state.memorials.length} 个名字</small></header>{state.memorials.length ? state.memorials.map((entry)=><article key={`${entry.survivorId}-${entry.day}`}><h2>{entry.name}</h2><span>DAY {entry.day} · {entry.cause}</span><p>{entry.epitaph}</p></article>) : <p className="v1r-empty">这里现在还是空的。</p>}<div className="v1r-civilian-note"><strong>街区居民的损失</strong><p>街区居民不逐个立档；有人死亡、失踪或离开，都会记进街区日志与人口统计。</p><small>居民离开 {state.campaignStats.civilianDepartures} · 死亡记录 {state.campaignStats.deaths}</small></div></section>}
    </main>
  );
}
