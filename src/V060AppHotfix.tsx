...
// ─────────────────────────────────────────────
// ART PLACEHOLDER — CSS silhouettes only.
// Building artwork, ruin textures, character illustrations will be
// injected via StreetSceneArt when canonical illustrations A01–Axx are ready.
//
// Injection point: replace each <div data-art-slot="…"> with
// <img src={AxxAsset} … /> per building slot.
// CSS controls: hotspot placement, grayscale, lighting, interaction states.
interface StreetSceneArtProps {
  stage: number; // 0–5 main light stage
  // Future: buildingAssets?: Partial<Record<BuildingId, string>>; // A01–Axx paths
}
function StreetSceneArt({ stage: _stage }: StreetSceneArtProps) {
  // CSS-only. Replace div children with <img> when A01–Axx art assets arrive.
  return (
    <>
      {/* ART SLOT bldg-far-l  → replace with A01-bldg-far-l.svg  */}
      <div className="v6-bldg v6-bldg--far-l"  data-art-slot="bldg-far-l"  />
      {/* ART SLOT bldg-left   → replace with A01-bldg-left.svg   */}
      <div className="v6-bldg v6-bldg--left"   data-art-slot="bldg-left"   />
      {/* ART SLOT bldg-center → replace with A01-bldg-center.svg */}
      <div className="v6-bldg v6-bldg--center" data-art-slot="bldg-center" />
      {/* ART SLOT bldg-right  → replace with A01-bldg-right.svg  */}
      <div className="v6-bldg v6-bldg--right"  data-art-slot="bldg-right"  />
      {/* ART SLOT bldg-far-r  → replace with A01-bldg-far-r.svg  */}
      <div className="v6-bldg v6-bldg--far-r"  data-art-slot="bldg-far-r"  />
      {/* ART SLOT wire        → replace with A01-wire.svg         */}
      <div className="v6-street__wire"          data-art-slot="wire"        />
      {/* ART SLOT main-light  → head replace with A01-lamp.svg   */}
...
// ─────────────────────────────────────────────
// Corrected BuildingId → display name → CSS class mapping:
//   clinic        诊疗站   cls: clinic
//   workshop      修理工坊 cls: workshop-bldg
//   searchStation 搜索站   cls: search
//   radio         广播亭   cls: radio-bldg
//   watchPost     守夜岗   cls: watch
//   shelter       宿营屋   cls: shelter-bldg
...
function StreetVisual({
  ...
  return (
    <section className={`v6-street v6-street--stage-${stage}`} aria-label="余烬长街">
      ...
      {/* Art placeholder layer — swap each slot for A01–Axx assets when ready */}
      ...
      {/* Building hotspots — real navigation, not fake */}
      {onBuildingClick && (
...
// ─────────────────────────────────────────────
// BUILDING FOCUS PANEL — shown when a hotspot is clicked
// ─────────────────────────────────────────────
        ...
        <>
          ...
          >
            {state.dayState.assignmentsLocked
              ? '今日派遣已锁定'
              : check.allowed
              ? `${level === 0 ? '开始修复' : `升级到 Lv${next.level}`}`
              : check.reason}
          </button>
        </>
      ) : (
...
// Street as interface: scene is the primary navigation surface.
// Panels appear contextually; nothing is fully expanded by default.
// ─────────────────────────────────────────────
function DayScreen({ state, setState }: { state: GameState; setState: (state: GameState) => void }) {
  ...
  // Building hotspot → panel navigation
  const [selectedBuilding, setSelectedBuilding] = useState<BuildingId | null>(null);
  // Assignment panel collapsed by default; expands on demand
  ...
  return (
    <main className="v6-shell">
      ...
      {/* ── SCENE: street is the interface, hotspots are real navigation ── */}
      <StreetVisual
        state={state}
      ...
      {/* ── BUILDING FOCUS: context panel, appears on hotspot click ── */}
      {selectedBuilding && (
        <BuildingFocusPanel
          id={selectedBuilding}
          state={state}
      ...
      {/* ── CONTEXTUAL: expedition in progress ── */}
      <ExpeditionStatus state={state} setState={setState} />

      {/* ── CONTEXTUAL: urgent missing persons ── */}
      {!state.dayState.assignmentsLocked && <MissingPanel state={state} setState={setState} />}

      {/* ── CONTEXTUAL: community support (only when residents exist) ── */}
      <CommunityPanel state={state} setState={setState} />

      {/* ── MEMORIAL ── */}
      <MemorialPanel state={state} />

      {/* ── SECONDARY: assignment panel, collapsed by default ── */}
      {!state.dayState.assignmentsLocked && (
        <div className="v6-section">
          <div className="v6-section__head">
            ...
            <button
              ...
              onClick={() => { setShowAssignments((v) => !v); setSelectedBuilding(null); }}
            >
      ...
      {/* ── FORECAST STRIP (replaces full two-column preview cards) ── */}
      <div className="v6-forecast-strip">
        ...
        <span className="v6-forecast-strip__detail">
          ...
          医疗 {prep.medical} · 维修 {prep.repair}
        </span>
      </div>
