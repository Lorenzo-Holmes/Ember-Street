...
Night content owns `audioKey: NightAudioKey`. The 51 static definitions and both dynamic mortality templates now resolve through 19 semantic event-audio categories. `night_infected_vocal` separates an approaching / rerouting infected mass from `night_horde_impact`, which remains the semantic cue for a barrier already taking physical hits. React still does not branch on individual night IDs for playback; `AudioDirector` reads the resolved event and `audioRegistry.ts` translates its semantic key into a local MP3. Five ambience layers cover day shelter, expedition, normal night, horde pressure and dawn/ending.

The audio runtime is deliberately outside `GameState`. Preferences live under `ember-street-audio-v1`; event de-duplication uses `sessionStorage` only so React remounts, menu navigation and same-tab refresh do not replay the same knock or crisis cue. No save migration or RNG draw is added. Audio unlock happens only after an explicit start/continue gesture. `visibilitychange` pauses ambience in the background. World/event cues keep the original mutually-exclusive SFX channel and ambience ducking; `page_turn`, `pen_circle` and `expedition_loot` use a short interaction channel with per-key cooldowns and no ambience ducking. Failed `HTMLAudioElement.play()` or missing MP3 files remain presentation failures rather than gameplay failures.
