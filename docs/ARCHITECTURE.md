...
Night content owns `audioKey: NightAudioKey`. The 51 static definitions and both dynamic mortality templates now resolve through 19 semantic event-audio categories. `night_infected_vocal` separates an approaching / rerouting infected mass from `night_horde_impact`, which remains the semantic cue for a barrier already taking physical hits. React still does not branch on individual night IDs for playback; `AudioDirector` reads the resolved event and `audioRegistry.ts` translates its semantic key into one source definition. Five ambience layers cover day shelter, expedition, normal night, horde pressure and dawn/ending.

The audio runtime is deliberately outside `GameState`. Preferences live under `ember-street-audio-v1`; event de-duplication uses `sessionStorage` only so React remounts, menu navigation and same-tab refresh do not replay the same knock or crisis cue. No save migration or RNG draw is added. Audio unlock happens only after an explicit start/continue gesture. `visibilitychange` suspends the active backend. World/event cues keep a mutually-exclusive SFX channel and ambience ducking; `page_turn`, `pen_circle` and `expedition_loot` use a short interaction channel with per-key cooldowns and no ambience ducking. Standard Web uses `HTMLAudioElement`. When `window.__EMBER_AUDIO_DATA__` is present, the same runtime switches to Web Audio, lazily decodes Base64 bytes with `decodeAudioData()`, keeps only the current ambience buffer and an eight-entry short-SFX LRU cache, and fails silent if Web Audio or decoding is unavailable.

`V1Entry` owns first-level navigation transitions. A fixed `home → buildings → survivors → records` order determines forward/backward paper movement; same-target navigation is a no-op, and the fixed bottom nav stays outside `NotebookPageTransition` so the control itself never moves with the paper. The CSS transition is about 220 ms and is removed under `prefers-reduced-motion`. `ExploreRouteV1` mounts a two-stroke SVG ellipse only for the active location; a real location change re-mounts it with dash-offset drawing and plays `pen_circle`, while clicking the current location does nothing. Expedition loot playback is decided from the post-resolution inventory delta, not from the chosen stance or result label.

Canonical runtime audio remains under `public/assets/audio/`. `scripts/audit-audio-assets.mjs` resolves every registry path, verifies MP3 signatures and enforces a 3.2 MiB aggregate source budget without hard-coding an expected file count. The five release BGM files are derived from the reviewed local 妙响 masters under ignored `music/`, imported with `scripts/import-reviewed-music.py`; `电流断续时` also supplies the dedicated power-failure excerpt. Core short cues remain deterministic sample-free project audio. Six tactile / recognizable cues (door knock, dogs, infected vocal, page turn, pen circle, expedition loot) come from six explicitly selected Mixkit Free License items through `scripts/import-free-sfx-v2.mjs`; raw WAV downloads live only in ignored `.audio-source-cache/`, while titles, item IDs, source pages, processing and runtime SHA-256 values are recorded in `docs/audio/SFX_SOURCE_LEDGER_V2.md`.

The Xiaohongshu mini-tool is a separate packaging backend because its upload whitelist excludes media-audio extensions. `scripts/build-minitool.mjs` parses the central registry, Base64-encodes all 30 registered MP3 byte streams into six classic JS data files (five BGM chunks plus one SFX chunk), rewrites registry file URLs to extensionless `embedded:` keys, injects those data scripts before the app bundle, removes the copied `assets/audio/` directory and emits `embedded-audio-manifest.json`. `scripts/audit-minitool.mjs` rejects `.mp3` outright, decodes every generated Base64 entry for the 1 MiB hard limit and verifies every `embedded:` reference has a payload. No network request, Blob media URL or `data:` media source is used.
...
- Web/Cloudflare: local MP3 ambience/SFX; Xiaohongshu mini-tool: Base64 JS payload + Web Audio decode; no streaming or runtime network audio
